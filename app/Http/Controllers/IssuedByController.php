...
class IssuedByController extends Controller
{
    public function index()
    {
        ...
        $createRoute = hasPermission('issuedBy') ? route('issuedBy.create') : '';
        $dataTableRoute = hasPermission('issuedBy') ? route('issuedBy.datatable') : '';

        return view('common.setup.issuedBy.index', get_defined_vars());
    }

    public function datatable()
    {
        $user = Auth::user();
        $query = IssuedBy::latest();
        ...
        return DataTables::of($query)
            ...
            ->addColumn('status', function ($row) {
                ...
                $statusUrl = route('issuedBy.status', ['id' => $row->id, 'status' => $newStatus]);
            ...
            ->addColumn('action', function ($row) {
                $editUrl   = route('issuedBy.edit', $row->id);
                $deleteUrl = route('issuedBy.destroy', $row->id);
    ...
    public function create()
    {
        ...
        $listRoute = hasPermission('issuedBy') ? route('issuedBy.index') : '';
        $saveRoute = hasPermission('issuedBy') ? route('issuedBy.store') : '';

        return view('common.setup.issuedBy.addEdit', get_defined_vars());
    }
    ...
    public function createAjax(Request $request)
    {
        ...
        $response = [
            'is_success' => 1,
            'icon' => 'success',
            'view' => view('common.setup.issuedBy.ajaxForm', get_defined_vars())->render()
        ];
    ...
    public function status($id, $status)
    {
        ...
        $user = Auth::user();
        $instructionSource = IssuedBy::where('id', $id)->first();
    ...
    public function edit($id)
    {
        ...
        $user = Auth::user();
        $listRoute = hasPermission('issuedBy') ? route('issuedBy.index') : '';
        $saveRoute = hasPermission('issuedBy') ? route('issuedBy.update', $id) : '';

        $editData = IssuedBy::where('id', $id)->first();
        if(empty($editData)){
            abort(404);
        }
    ...
    public function destroy($id)
    {
        ...
        $user = Auth::user();
        $data = IssuedBy::where('id', $id)->first();
        if(empty($data)){
            return [
                'is_success' => 0,
                'icon' => 'error',
    ...
    public function saveData(Request $request, $id = null)
    {
        ...
        if (empty($instructionSource)) {
            $instructionSource = new IssuedBy();
        }
