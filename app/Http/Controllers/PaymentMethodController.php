...
class PaymentMethodController extends Controller
{
    public function index()
    {
        ...
        $createRoute = hasPermission('paymentMethod') ? route('paymentMethod.create') : '';
        $dataTableRoute = hasPermission('paymentMethod') ? route('paymentMethod.datatable') : '';

        return view('common.setup.paymentMethod.index', get_defined_vars());
    }
    ...
    public function datatable()
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('status', function ($row) {
                ...
                $statusUrl = route('paymentMethod.status', ['id' => $row->id, 'status' => $newStatus]);
            ...
            ->addColumn('action', function ($row) {
                $editUrl   = route('paymentMethod.edit', $row->id);
                $deleteUrl = route('paymentMethod.destroy', $row->id);
    ...
    public function create()
    {
        ...
        $listRoute = hasPermission('paymentMethod') ? route('paymentMethod.index') : '';
        $saveRoute = hasPermission('paymentMethod') ? route('paymentMethod.store') : '';

        return view('common.setup.paymentMethod.addEdit', get_defined_vars());
    }
    ...
    public function createAjax(Request $request)
    {
        ...
        $response = [
            'is_success' => 1,
            'icon' => 'success',
            'view' => view('common.setup.paymentMethod.ajaxForm', get_defined_vars())->render()
        ];
    ...
    public function status($id, $status)
    {
        ...
        $user = Auth::user();
        $instructionSource = PaymentMethod::where('id', $id)->first();
    ...
    public function edit($id)
    {
        ...
        $user = Auth::user();
        $listRoute = hasPermission('paymentMethod') ? route('paymentMethod.index') : '';
        $saveRoute = hasPermission('paymentMethod') ? route('paymentMethod.update', $id) : '';
    ...
    public function saveData(Request $request, $id = null)
    {
        ...
        if (empty($instructionSource)) {
            $instructionSource = new PaymentMethod();
        }
        ...
        try {
            ...
            if ((!isset($id) || empty($id)) && !$request->is_ajax_modal) {
                $response['redirect_url'] = route('paymentMethod.index');
            }
            
            return $response;
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('PaymentMethod store error', ['error' => $e->getMessage()]);
