...
class CardOwnerController extends Controller
{
    public function index()
    {
        ...
        $createRoute = hasPermission('cardOwner') ? route('cardOwner.create') : '';
        $dataTableRoute = hasPermission('cardOwner') ? route('cardOwner.datatable') : '';

        return view('common.setup.cardOwner.index', get_defined_vars());
    }

    public function datatable()
    {
        $user = Auth::user();
        $query = CardOwner::latest();
        ...
        return DataTables::of($query)
            ...
            ->addColumn('status', function ($row) {
                ...
                $statusUrl = route('cardOwner.status', ['id' => $row->id, 'status' => $newStatus]);
            ...
            ->addColumn('action', function ($row) {
                $editUrl   = route('cardOwner.edit', $row->id);
                $deleteUrl = route('cardOwner.destroy', $row->id);
    ...
    public function create()
    {
        ...
        $listRoute = hasPermission('cardOwner') ? route('cardOwner.index') : '';
        $saveRoute = hasPermission('cardOwner') ? route('cardOwner.store') : '';

        return view('common.setup.cardOwner.addEdit', get_defined_vars());
    }
    ...
    public function createAjax(Request $request)
    {
        ...
        $response = [
            'is_success' => 1,
            'icon' => 'success',
            'view' => view('common.setup.cardOwner.ajaxForm', get_defined_vars())->render()
        ];
    ...
    public function status($id, $status)
    {
        ...
        $user = Auth::user();
        $instructionSource = CardOwner::where('id', $id)->first();
    ...
    public function edit($id)
    {
        ...
        $user = Auth::user();
        $listRoute = hasPermission('cardOwner') ? route('cardOwner.index') : '';
        $saveRoute = hasPermission('cardOwner') ? route('cardOwner.update', $id) : '';

        $editData = CardOwner::where('id', $id)->first();
        if(empty($editData)){
            abort(404);
        }
    ...
    public function destroy($id)
    {
        ...
        $user = Auth::user();
        $data = CardOwner::where('id', $id)->first();
        if(empty($data)){
            return [
                'is_success' => 0,
                'icon' => 'error',
    ...
    public function saveData(Request $request, $id = null)
    {
        ...
        if (empty($instructionSource)) {
            $instructionSource = new CardOwner();
        }
        ...
        try {
            ...
            if ((!isset($id) || empty($id)) && !$request->is_ajax_modal) {
                $response['redirect_url'] = route('cardOwner.index');
            }
            
            return $response;
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('CardOwner store error', ['error' => $e->getMessage()]);
