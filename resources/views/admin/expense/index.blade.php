		...
		<div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
			<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
				...
				<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
					...
					<li class="breadcrumb-item">{{ $getCurrentTranslation['expense_list'] ?? 'expense_list' }}</li>
				</ul>
			</div>
			<div class="d-flex align-items-center gap-2 gap-lg-3">
				@if(isset($createRoute) && !empty($createRoute))
					<a href="{{ $createRoute }}" class="btn btn-sm fw-bold btn-primary">{{ $getCurrentTranslation['add_new_expense'] ?? 'add_new_expense' }}</a>
				@endif
			</div>
		</div>
	</div>
			...
			<div class="card rounded border mt-5 p-10 bg-white">
				<div class="card-header p-0" style="min-height: unset">
					<h3 class="card-title mb-3 mt-0">
						{{ $getCurrentTranslation['expense_list'] ?? 'expense_list' }}
					</h3>
				</div>
				<table id="datatable" class="table table-rounded table-striped border gy-7 gs-7">
					<thead>
						<tr>
							<th>#</th>
							<th>{{ $getCurrentTranslation['expense_category'] ?? 'expense_category' }}</th>
							<th>{{ $getCurrentTranslation['for'] ?? 'for' }}</th>
							<th>{{ $getCurrentTranslation['title'] ?? 'title' }}</th>
							<th>{{ $getCurrentTranslation['amount'] ?? 'amount' }}</th>
							<th>{{ $getCurrentTranslation['expense_date'] ?? 'expense_date' }}</th>
							<th>{{ $getCurrentTranslation['payment_method'] ?? 'payment_method' }}</th>
...
var dataTable = $('#datatable').DataTable({
	...
	columns: [
		...
		{ data: 'for_user_name', name: 'for_user_name', orderable: false, searchable: true },
		{ data: 'title', name: 'title' },
		{ data: 'amount_formatted', name: 'amount', orderable: true, searchable: false },
		{ data: 'expense_date_formatted', name: 'expense_date', orderable: true, searchable: false },
