			...
			<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
				...
				<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
					...
					@if(isset($listRoute) && !empty($listRoute))
						<li class="breadcrumb-item text-muted">
							<a href="{{ $listRoute }}" class="text-muted text-hover-primary">{{ $getCurrentTranslation['designation_list'] ?? 'designation_list' }}</a> &nbsp; - 
						</li>
					@endif
					@if(isset($editData))
						<li class="breadcrumb-item">{{ $getCurrentTranslation['edit_designation'] ?? 'edit_designation' }}</li>
					@else
						<li class="breadcrumb-item">{{ $getCurrentTranslation['create_designation'] ?? 'create_designation' }}</li>
					@endif
				</ul>
			</div>
				...
				<div class="col-md-8 col-lg-6 m-auto">
					<div class="card rounded border mt-5 bg-white append-item-container">
						<div class="card-header">
							@if(isset($editData))
								<h3 class="card-title">{{ $getCurrentTranslation['edit_designation'] ?? 'edit_designation' }}</h3>
							@else
								<h3 class="card-title">{{ $getCurrentTranslation['create_designation'] ?? 'create_designation' }}</h3>
							@endif
							...
							<div class="row">
								<div class="col-md-12">
									<div class="form-item mb-5">
										...
										<input type="text" class="form-control" placeholder="{{ $getCurrentTranslation['enter_designation_name'] ?? 'enter_designation_name' }}" name="name" ip-required value="{{ old('name') ?? $editData->name ?? '' }}"/>
										@error('name')
