	...
	<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
		<div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
			...
			<div class="d-flex align-items-center gap-2 gap-lg-3">
				@php
					$exportUrl = route('admin.attendance.exportPdf', request()->all());
				@endphp
				...
				<div class="card-body px-0">
					...
					@php
						...
						foreach ($attendances as $attendance) {
							...
							if ($isRunning) {
								$totalRunningHours += ($attendance->running_total_hour ?? 0);
							}
							if ($attendance->check_out) {
								$totalCompletedHours += ($attendance->total_hours ?? 0);
							}
							if ($attendance->total_hours > $dailyWorkTime) {
								$totalOvertimeHours += ($attendance->total_hours - $dailyWorkTime);
							}
						}
					@endphp
					<div class="row">
									...
									<div class="table-responsive">
										<table class="report-table table table-bordered table-striped table-hover align-middle mb-0">
											...
											<tfoot class="table-secondary">
												<tr>
													<td colspan="7" class="fw-bold text-end bg-secondary">
														{{ $getCurrentTranslation['total_label'] ?? 'total_label' }}
													</td>
													<td class="fw-bold bg-secondary text-primary">
														{{ formatHoursMinutes($totalRunningHours) }}
													</td>
													<td class="fw-bold bg-secondary text-success">
														{{ formatHoursMinutes($totalCompletedHours) }}
													</td>
													<td class="fw-bold bg-secondary">
														{{-- {{ $attendances->count() }} --}}
													</td>
													<td class="fw-bold bg-secondary text-warning">
										...
										<table class="report-table table table-bordered table-striped table-hover align-middle mb-0">
											<thead class="table-secondary">
												<tr>
													...
													<th class="fw-semibold" style="width: 140px">{{ $getCurrentTranslation['work_hours_gap_present'] ?? 'Work Hours Gap (Present Days)' }}</th>
													<th class="fw-semibold" style="width: 150px">{{ $getCurrentTranslation['work_hours_gap_total'] ?? 'Work Hours Gap (Present + Absent Days)' }}</th>
													<th class="fw-semibold">{{ $getCurrentTranslation['action'] ?? 'action' }}</th>
												</tr>
											</thead>
											<tbody>
											...
											<tfoot class="table-secondary">
												<tr>
													<td colspan="3" class="fw-bold bg-secondary text-end">
														<strong>{{ $getCurrentTranslation['total_label'] ?? 'Total' }}:</strong>
													</td>
													<td class="fw-bold bg-secondary">
														<strong>{{ $userSummaryTotals['total_present_days'] }}</strong>
													</td>
													<td class="fw-bold bg-secondary">
														<strong>{{ $userSummaryTotals['total_absent_days'] }}</strong>
													</td>
													<td class="fw-bold bg-secondary">
														<strong>{{ formatHoursMinutes($userSummaryTotals['total_work_hours']) }}</strong>
													</td>
													<td class="fw-bold bg-secondary">
														<strong>{{ formatHoursMinutes($userSummaryTotals['total_overtime_hours']) }}</strong>
													</td>
													<td class="fw-bold bg-secondary">
														@php
															$totalGapPresent = $userSummaryTotals['work_hours_gap_present'] ?? 0;
															$totalGapPresentClass = $totalGapPresent > 0 ? 'text-warning' : 'text-success';
														@endphp
														<span class="{{ $totalGapPresentClass }}">
															<strong>{{ formatHoursMinutes($totalGapPresent) }}</strong>
														</span>
													</td>
													<td class="fw-bold bg-secondary">
														@php
															$totalGapTotal = $userSummaryTotals['work_hours_gap_total'] ?? 0;
															$totalGapTotalClass = $totalGapTotal > 0 ? 'text-warning' : 'text-success';
														@endphp
														<span class="{{ $totalGapTotalClass }}">
															<strong>{{ formatHoursMinutes($totalGapTotal) }}</strong>
														</span>
													</td>
													<td class="bg-secondary"></td>
												</tr>
