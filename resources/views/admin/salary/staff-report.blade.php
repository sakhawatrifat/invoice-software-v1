	...
	<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
		<div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
			<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
				...
				<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
					...
					@if($employee->is_staff == 1)
						<li class="breadcrumb-item text-muted">
							<a href="{{ route('staff.index') }}" class="text-muted text-hover-primary">{{ $getCurrentTranslation['staff_list'] ?? 'staff_list' }}</a> &nbsp; - 
						</li>
						<li class="breadcrumb-item text-muted">
							<a href="{{ route('staff.show', $employee->id) }}" class="text-muted text-hover-primary">{{ $employee->name }}</a> &nbsp; - 
						</li>
					@else
						<li class="breadcrumb-item text-muted">
							<a href="{{ route('user.index') }}" class="text-muted text-hover-primary">{{ $getCurrentTranslation['user_list'] ?? 'user_list' }}</a> &nbsp; - 
						</li>
						<li class="breadcrumb-item text-muted">
							<a href="{{ route('user.show', $employee->id) }}" class="text-muted text-hover-primary">{{ $employee->name }}</a> &nbsp; - 
						</li>
					@endif
			...
			<div class="d-flex align-items-center gap-2 gap-lg-3">
				@php
					$exportPdfUrl = route('admin.salary.staffReportExportPdf', ['id' => $employee->id]) . '?' . http_build_query(request()->all());
					$exportCsvUrl = route('admin.salary.staffReportExportCsv', ['id' => $employee->id]) . '?' . http_build_query(request()->all());
				@endphp
				{{-- <a href="{{ $exportCsvUrl }}" class="btn btn-sm fw-bold btn-info" target="_blank">
					<i class="fas fa-file-excel"></i> {{ $getCurrentTranslation['export'] ?? 'Export' }}
				</a> --}}
				<a href="{{ $exportPdfUrl }}" class="btn btn-sm fw-bold btn-danger" target="_blank">
					<i class="fas fa-file-pdf"></i> {{ $getCurrentTranslation['export_pdf'] ?? 'Export PDF' }}
				</a>
				@if($employee->is_staff == 1)
					<a href="{{ route('staff.show', $employee->id) }}" class="btn btn-sm fw-bold btn-secondary">
						<i class="fa-solid fa-arrow-left"></i> {{ $getCurrentTranslation['back'] ?? 'Back' }}
					</a>
				@else
					<a href="{{ route('user.show', $employee->id) }}" class="btn btn-sm fw-bold btn-secondary">
						<i class="fa-solid fa-arrow-left"></i> {{ $getCurrentTranslation['back'] ?? 'Back' }}
					</a>
				@endif
			</div>
