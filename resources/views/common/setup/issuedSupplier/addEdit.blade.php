			...
			<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
				...
				<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
					...
					@if(isset($listRoute) && !empty($listRoute))
						<li class="breadcrumb-item text-muted">
							<a href="{{ $listRoute }}" class="text-muted text-hover-primary">{{ $getCurrentTranslation['issued_supplier_list'] ?? 'issued_supplier_list' }}</a> &nbsp; - 
						</li>
					@endif
					@if(isset($editData))
						<li class="breadcrumb-item">{{ $getCurrentTranslation['edit_issued_supplier'] ?? 'edit_issued_supplier' }}</li>
					@else
						<li class="breadcrumb-item">{{ $getCurrentTranslation['create_issued_supplier'] ?? 'create_issued_supplier' }}</li>
					@endif
				</ul>
			</div>
				...
				<div class="col-md-8 col-lg-6 m-auto">
					<div class="card rounded border mt-5 bg-white append-item-container">
						<div class="card-header">
							@if(isset($editData))
								<h3 class="card-title">{{ $getCurrentTranslation['edit_issued_supplier'] ?? 'edit_issued_supplier' }}</h3>
							@else
								<h3 class="card-title">{{ $getCurrentTranslation['create_issued_supplier'] ?? 'create_issued_supplier' }}</h3>
							@endif
