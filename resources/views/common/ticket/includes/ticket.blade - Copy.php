   ...
   @if(isset($withPrice) && $withPrice == 1)
      <div style="page-break-inside: avoid;">
         <div class="inv-section" style="margin-top: 20px;">
            ...
            {{-- <h3 style="font-family: {{ language_font(strip_tags($getCurrentTranslation['invoice_summary'] ?? 'invoice_summary')) }}; background-color: #e7e7e7; padding: 5px 10px; margin: 0; font-size: 15px; color: #32323b;">{{ $getCurrentTranslation['invoice_summary'] ?? 'invoice_summary' }}</h3> --}}
            <div class="inv-table-wrapper" style="padding: 10px; border: 1px solid #e5e5e5;">
               <table style="width: 100%; border-collapse: collapse;">
                  <thead>
                     <tr>
                        <th style="font-family: {{ language_font(strip_tags($getCurrentTranslation['pax_type_label'] ?? 'pax_type_label'), true) }}; color: #32323b; padding: 10px; font-size: 14px; text-align: left;">{{ $getCurrentTranslation['pax_type_label'] ?? 'pax_type_label' }}</th>
                  ...
                  <tbody>
                     @foreach($editData->fareSummary as $key => $fare)
                        <tr>
                           <td style="font-family: {{ language_font(strip_tags($fare->pax_type ?? '')) }}; color: #474751; padding: 10px; font-size: 14px;">{{ $fare->pax_type }}</td>
                           <td style="color: #474751; padding: 10px; font-size: 14px;">
