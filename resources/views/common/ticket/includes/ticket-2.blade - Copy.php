...
<div class="inv-content-wrapper" style="color: #32323b; background: #fff;">
   ...
   <div class="inv-section" style="margin-top: 20px;">
      <table style="width: 100%; border-collapse: collapse; vertical-align: middle;">
         <tr>
            <td style="font-size: 15px; color: #32323b; font-weight: bold;padding: 5px 10px;">
               @if(!empty(env('DB_PASSWORD')) || (isset($view)) && $view == 1)
                  <img src="{{ $editData->flights[0]->airline->logo_url ?? '' }}" alt="{{ $editData->flights[0]->airline->name ?? '' }}" style="width: auto; max-width: 200px; vertical-align:middle; margin-right:10px;">
               @else
                  <h4 style="text-align: left; margin: 0; color: #32323b;">Airlines Logo Here</h4>
               @endif
            </td>
               ...
               <table align="center" style="margin: 0 auto;">
                  <tr>
                     <td style="text-align: left;">
                        <strong style="color: #6c6c6c; font-weight: normal;">Booking ID:</strong><br>
                        <span>{{ $editData->reservation_number ?? 'N/A' }}</span><br>
                     </td>
                  </tr>
               </table>
            </td>
            ...
            <td style="text-align: right;padding: 5px 10px;">
               <strong style="color: #6c6c6c; font-weight: normal;">Booking Date:</strong><br>
               @if(!empty($editData->invoice_date))
                  <span>{{ date('d M Y', strtotime($editData->invoice_date)) }}</span>
               @else
                  N/A
               @endif
            </td>
   ...
   <div class="inv-section flight-itineraries-table" style="margin-top: 20px;">
      <table style="width: 100%; border-collapse: collapse; vertical-align: middle; background-color: transparent">
         <tr>
            <th style="text-align: left; font-size: 18px; color: #32323b; font-weight: bold;padding: 10px 0px; background-color: #ffffff;">
              Travel Itineraries
            </th>
         </tr>
      </table>
      @foreach($editData->flights as $key => $flight)
         <table style="page-break-inside: avoid; width: 100%; border-collapse: collapse; vertical-align: middle; background-color: #e4e4e4; border: 1px solid #e4e4e4; margin-bottom: {{ $loop->last ? '5px' : '20px' }}">
            <tr>
               <th style="width: 205px; text-align: left; font-size: 15px; color: #32323b; font-weight: bold;padding: 15px 10px;">
                  Flight {{ $key+1 }}
               </th>
               ...
               <th style="text-align: right; font-size: 15px; color: #32323b; font-weight: bold;padding: 15px 10px;">
               ...
               <td colspan="3" style="padding: 30px 20px 30px 20px; background-color: #ffffff; border: 1px solid #e4e4e4">
                  <table style="width:100%; border-collapse: collapse; vertical-align: top; background-color: #ffffff" >
                     ...
                     <tr>
                        ...
                        <td style="text-align: right; background-color: #ffffff">
                           @if($departureDate != $arrivalDate)
                              <span style="color: red">Arrives next day</span>
                           @endif
                        </td>
                     </tr>
                  </table>
                  <table style="width: 100%; vertical-align: middle;">
                     <tr>
                        ...
                        <td style="background-color: #ffffff; padding: 20px 10px 0px 10px;">
                           {{-- {{  $flight->airline->name }} --}}
                           {{ $flight->flight_number }}
                        </td>
                        <td style="background-color: #ffffff; padding: 20px 0px 0px 10px; text-align: right;">{{ $flight->total_fly_time }}</td>
                     </tr>
                  </table>
               </td>
            </tr>
            ...
            @foreach($flight->transits as $tKey => $transit)
               <tr>
                  <th style="width: 205px; text-align: left; font-size: 15px; color: #008000; background: #f4f4f4; border-left: 1px solid #e4e4e4; font-weight: bold;padding: 15px 10px;">
                  ...
                  <th style="text-align: right; font-size: 15px; color: #008000; background: #f4f4f4; border-right: 1px solid #e4e4e4; font-weight: bold;padding: 15px 10px;">
                  ...
                  <td colspan="3" style="padding: 30px 20px 30px 20px; background-color: #ffffff; border: 1px solid #e4e4e4">
                     ...
                     <table style="width: 100%; vertical-align: middle;">
                        <tr>
                           ...
                           <td style="background-color: #ffffff; padding: 20px 0px 0px 10px; text-align: right;">{{ $transit->total_fly_time }}</td>
                        </tr>
                     </table>
                  </td>
               </tr>
      ...
      @endforeach
      <span>All times shown are local time</span>
   </div>
