...
<div class="d-flex flex-column flex-column-fluid">
	...
	<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
		<div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
			...
			<div class="d-flex align-items-center gap-2 gap-lg-3">
				@if(isset($editData) && !empty($editData) && $editData->document_type == 'ticket')
					<div class="btn-group">
				...
				@if(isset($editData) && !empty($editData) && count($editData->fareSummary))
					<a href="{{ route('ticket.downloadPdf', $editData->id) }}?invoice=1&withPrice=1" class="btn btn-sm fw-bold btn-primary">{{ $getCurrentTranslation['download_invoice'] ?? 'download_invoice' }}</a>
				@endif
	...
	<div id="kt_app_content" class="app-content flex-column-fluid">
		<div id="kt_app_content_container" class="app-container container-fluid">
			<form class="" method="post" action="{{ route('ticket.mailSend', $editData->id) }}" enctype="multipart/form-data">
				@csrf
                @method('put')
                ...
                <div class="card rounded border mt-5 bg-white append-item-container">
					...
					<div class="card-body">
						<div class="row">
							<div class="col-md-12">
								<label class="form-label">{{ $getCurrentTranslation['select_passenger'] ?? 'select_passenger' }}:</label>
								@if(count($editData->passengers) > 0)
								...
								<div class="d-flex align-items-center form-item mb-4">
									@if(isset($editData) && !empty($editData) && $editData->document_type == 'ticket')
										<div class="mb-2 pe-8">
											<div class="form-check">
												<label class="form-check-label user-select-none">
													<input type="checkbox" class="form-check-input group-checkbox" name="document_type_ticket" value="1" checked>
													{{ $getCurrentTranslation['ticket'] ?? 'ticket' }}
												</label>
											</div>
										</div>
								...
								<div class="d-flex align-items-center form-item mb-4">
									@if(isset($editData) && !empty($editData) && count($editData->fareSummary))
										<div class="mb-2 pe-6">
											<div class="form-check">
												<label class="form-check-label user-select-none">
													<input type="checkbox" class="form-check-input group-checkbox" name="document_type_invoice" value="1">
										...
										{{-- <div class="mb-2">
											<div class="form-check">
												<label class="form-check-label user-select-none">
													<input type="checkbox" class="form-check-input group-checkbox" name="invoice_with_price" value="1">
													{{ $getCurrentTranslation['invoice'] ?? 'invoice' }}
													{{ $getCurrentTranslation['with_price'] ?? 'with_price' }}
												</label>
											</div>
										...
										<div class="d-flex align-items-center form-item mb-4 gap-3">
											...
											<div class="ticket-layout-card-outer">
												<label class="ticket-layout-card mb-1">
													...
													<img width="120" src="{{ asset('assets/images/ticket-layout/ticket-layout-1.png') }}" class="ticket-img">
												</label>
											...
											<div class="ticket-layout-card-outer">
												<label class="ticket-layout-card mb-1">
													...
													<img width="120" src="{{ asset('assets/images/ticket-layout/ticket-layout-2.png') }}" class="ticket-img">
												</label>
											...
											<div class="ticket-layout-card-outer">
												<label class="ticket-layout-card mb-1">
													...
													<img width="120" src="{{ asset('assets/images/ticket-layout/ticket-layout-3.png') }}" class="ticket-img">
												</label>
...
<script>
	...
	$(document).on('click', '.customer-name-checkbox', function (e) {
		...
		$.ajax({
			url: "{{ route('ticket.mailContentLoad', $editData->id) }}",
			type: 'POST',
			data: formData,
